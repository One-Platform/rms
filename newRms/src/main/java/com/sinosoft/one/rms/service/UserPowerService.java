...
//				geRmsExcPowerRepository.delete(excPowers);
	...
	public List<DataRule> findAllDataRule(){
		return (List<DataRule>) geRmsDataRuleRepository.findAll();
	}
	
	//取得在人员数据权限表中相应的数据规则
	public List<DataRule> getUserDataRule(String comCode, String userCode){
		...
		for(BusPower busPower : busPowers){
			dataRules.add(busPower.getDataRule());
		}
	...
	public List<BusPower> getParams(String comCode, String userCode,
		...
		String userPowerId = geRmsUserPowerRepository.findIdByUserCodeComCode(userCode, comCode);
		List<String> busPowerIds = geRmsBusPowerRepository.findBusPowerIdByUserPowerIdTaskId(userPowerId, Arrays.asList(dataRuleIds));
...
//			System.out.println("记录数："+busPowers.size());
	 ...
	 * @return
	 */
	public List<Company> findPowerCompanyByuserCode(String userCode){
		List<Company> companies=companyDao.findPowerCompanyByuserCode(userCode);
		return companies;
	}
}
